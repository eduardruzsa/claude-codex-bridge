...
- **Socket ownership:** a Linux abstract-namespace socket acts as the lock for each label. Its name is keyed with the private token, so other users can't compute it. Only one process can bind it, and the kernel releases it when that process dies. That leaves no stale lock to recover and no race between two starting processes. On top of the lock:
  - An existing socket file is replaced only when nothing accepts connections on it. This also covers an owner the lock can't see, such as one in another network namespace.
  - Exit cleanup removes the socket file only if it is still the same file (same inode).
  - A process that fails to listen releases the label.

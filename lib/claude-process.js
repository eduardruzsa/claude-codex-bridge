...
// CC_BRIDGE_CLAUDE_PROC=none turns detection off (tests run inside real Claude sessions).
export function findClaudeProcess(from = process.ppid) {
  if (process.env.CC_BRIDGE_CLAUDE_PROC === 'none') return null
  let pid = from
  for (let depth = 0; pid > 1 && depth < 10; depth++) {
    ...
    // comm alone isn't enough: a runtime may rename its main thread (Node 24+ sets
    // "node-MainThread"), so the program name in argv[0] counts too.
    let args = []
    try {
      args = fs.readFileSync(`/proc/${pid}/cmdline`, 'utf8').split('\0').filter(Boolean)
    } catch {}
    if (s.comm === 'claude' || path.basename(args[0] || '') === 'claude') return { pid, start: s.start, args }
    pid = s.ppid
  }
  return null
}

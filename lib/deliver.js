...
const codexBin = () => process.env.CC_BRIDGE_CODEX_BIN || 'codex'
...
export function deliverToCodex({ fromLabel, thread, text, replyToId, kind = 'message' }) {
  const msgId = newId()
  const base = { msg_id: msgId, from: claudeParty(fromLabel), to: codexParty(thread), reply_to: replyToId || null, kind }
...
// Codex → Claude via the live session's channel socket.
export async function deliverToClaude({ fromThread, label, text, replyToId, kind = 'message' }) {
  const msgId = newId()
  const base = { msg_id: msgId, from: codexParty(fromThread), to: claudeParty(label), reply_to: replyToId || null, kind }
  record({ event: 'sent', ...base, text })

...
export function deliverToCodex({ fromLabel, claudeSession, thread, text, replyToId, kind = 'message', msg_id = newId(), created_at = new Date().toISOString() }) {
  const msgId = msg_id
  ...
  record({ event: 'sent', ...base, text, created_at })
    ...
    const child = execFile(
      ...
      (err, stdout, stderr) => {
        if (err) {
          ...
          const status = err.killed || err.signal ? 'unconfirmed' : 'failed'
          record({ event: status, msg_id: msgId, error })
          resolve({ status, msg_id: msgId, error })
          return
        }
...
export async function deliverToClaude({ fromThread, label, claudeSession, text, replyToId, kind = 'message', msg_id = newId(), created_at = new Date().toISOString() }) {
  const msgId = msg_id
  ...
  record({ event: ['disconnected', 'session_changed', 'transitioning'].includes(res.status) ? 'failed' : 'unconfirmed', msg_id: msgId, error: res.error || res.status })

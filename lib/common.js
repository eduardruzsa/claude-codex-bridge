...
// Pairings are strictly one-to-one: Claude label ⇄ Codex thread UUID.
export function loadPairs() {
  return readJson(paths.pairs(), {})
}

export function pairSessions(label, thread) {
  if (!validLabel(label)) throw new Error(`invalid Claude label: ${label}`)
  if (!/^[0-9a-f-]{36}$/i.test(thread)) throw new Error(`invalid Codex thread id: ${thread}`)
  const pairs = loadPairs()
  const replaced = []
  for (const [l, t] of Object.entries(pairs)) {
    if (l === label || t === thread) {
      replaced.push({ claude: l, codex: t })
      delete pairs[l]
    }
  }
  pairs[label] = thread
...
export function labelForThread(thread) {
  return Object.entries(loadPairs()).find(([, t]) => t === thread)?.[0]
}
...
export function claudeSessionAlive(label) {
  return sendToClaudeSocket(label, { op: 'ping' }, 1500).then(r => r.status === 'ok')
}

...
function installCodexPlanHook(home) {
  ...
  config.hooks ||= {}
  // Drop an older cc-bridge plan hook (e.g. from a moved checkout), then add ours.
  const stop = (config.hooks.Stop || [])
    .map(g => ({ ...g, hooks: (g.hooks || []).filter(h => !/plan-review["']? --codex$/.test(h.command || '')) }))
    .filter(g => g.hooks.length)
  stop.push({ hooks: [{ type: 'command', command: planHookCommand(), timeout: 600 }] })
...
export function codexPlanHookTrust(timeoutMs = 15000) {
  return new Promise(resolve => {
    ...
    send({ id: 1, method: 'initialize', params: { clientInfo: { name: 'cc-bridge-doctor', version: '0.3.1' } } })
  })
}
...
export function install({ home = os.homedir() } = {}) {
  ...
  installPlugin()
  installCodexPlanHook(home)
  return 'Installed. Restart Codex once and trust the new cc-bridge plan-review hook when it asks (or run /hooks);\n' +
    'Codex skips hooks until you review them. Start Claude with claude-live, then ask either agent to talk to the other.'
}
export async function doctor() {
  ...
  checks.push({ name: 'Codex plan-review hook', ok: codexPlanHookInstalled(), fix: 'Run: cc-bridge install' })

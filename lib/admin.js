...
export function install({ home = os.homedir() } = {}) {
  ...
  return 'Installed.\n1. Restart Codex.\n2. Start claude-live in your project and approve its channel prompt.\n3. Ask either agent to send a short question to the other; approve bridge tools when prompted.\nUse cc-bridge status to see waiting requests and replies.' +
    (reviewCodex
...
// What the bridge creates in its data and runtime directories. --purge deletes only
// entries with these exact name patterns, and a subdirectory only if everything in it
// matches too; the directory itself goes only if it ends up empty. A custom data_dir
// or runtime_dir should still be dedicated to the bridge (a file named "token" in it
// would be taken for ours).
const TMP = String.raw`\.\d+\.tmp`
const OWNED_FILES = [/^token$/, /^transcript\.jsonl$/, new RegExp(`^(pairs|consultations)\\.json(${TMP})?$`),
  /^claude-[A-Za-z0-9_-]{1,40}\.sock$/, /^doctor-\d+\.sock$/]
const OWNED_DIRS = [
  [/^launches$/, /^[0-9a-f-]{36}\.json(\.\d+\.[0-9a-f-]{36}\.tmp)?$/],
  [/^pending-(codex|claude)$/,  new RegExp(`^[A-Za-z0-9_-]{1,40}\\.json(${TMP}|\\.\\d+\\.[0-9a-f-]{36}\\.tmp)?$`)],
  [/^plan-review$/, /^claude-[0-9a-f]{32}\.json$/],
  [/^proc-\d+-\d+$/, new RegExp(`^state\\.(json(${TMP})?|lock)$`)],
]
function purgeDir(dir) {
  ...
  for (const e of entries) {
    ...
    if (e.isDirectory()) {
      const inner = OWNED_DIRS.find(([re]) => re.test(e.name))?.[1]
      if (!inner) continue
      const contents = fs.readdirSync(full, { withFileTypes: true })
      if (!contents.every(c => !c.isDirectory() && inner.test(c.name))) continue
      fs.rmSync(full, { recursive: true, force: true })
      removed.push(full)
    } else if (OWNED_FILES.some(re => re.test(e.name))) {

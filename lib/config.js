...
// Writes the defaults if there is no config file yet. Returns false if one exists.
export function initConfig() {
  const file = configPath()
  if (fs.existsSync(file)) return false
  writeFile(file, defaultsJson())
  return true
}
...
export function setConfig(key, raw) {
  ...
  const file = configPath()
  const data = put(structuredClone(fileData(file)) || JSON.parse(defaultsJson()), key, value)

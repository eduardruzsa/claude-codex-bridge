...
export const START_TIMEOUT_MS = 3 * 60 * 1000
...
// Runs `argv` in a new terminal window in `cwd`. CC_BRIDGE_TERMINAL overrides the
// terminal command (it receives argv, with cwd set). Env vars are passed through
// `env` inside argv because a single-instance terminal may not inherit ours.
function launchInTerminal(cwd, title, argv) {
  const custom = process.env.CC_BRIDGE_TERMINAL
  const [cmd, ...prefix] = custom
...
export function launchCodex(cwd, codexArgs) {
  return launchInTerminal(cwd, 'Codex (cc-bridge)', [process.env.CC_BRIDGE_CODEX_BIN || 'codex', ...codexArgs])
}
...
export function addPending(kind, label, key, text, now = Date.now()) {
  ...
  const starting = same && now - current.started_at < START_TIMEOUT_MS
  const next = starting
